		...
		$(function(){
			...
			$("input:checkbox[name=checkAll]").click(function(){
				if($(this).is(":checked")==true){
					$("input:checkbox[name=prdCheck]").prop("checked", true);
				} else {
					$("input:checkbox[name=prdCheck]").prop("checked", false);
				}
			});
...
<!-- 									<input type="hidden" name="buyPrdId" value="0"/> -->
									<table width="100%" class="content_title_box2">
										...
										<tbody>
											...
											%>	
												<tr class="mOver">
													<td><input type="checkbox" class="check_box" name="prdCheck"/></td>
													<td>
...
<%-- 														<input type="hidden" name="goodsId" value="<%=cDto.getGoodsId() %>"/> --%>
									...
									<div class="continue_btn_wrap">
										...
										<button onclick="action='EShopBuy.jsp?'" type="submit" name="cBtn" value="buySel">
