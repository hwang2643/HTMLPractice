...
//			}																						
			this.canvas.onmousemove = function(event){	
				...
				if((x>=238 && x<=282) && (y>=318 && 370>=y)){																					
					tooltip.textContent = "동측G구역 [동측지정석] 0석";																				
		...
		function draw() {			
			...
			this.canvasDraw.onmousemove = function(event){
				...
				document.addEventListener("mousemove", (e) => {
					for(let i=0; i<=jArr.lenght; i++){
						if((x>=jArr[i].x_start && x<=jArr[i].x_end) && (y>=jArr[i].y_start && y<=jArr[i].y_end)){
							const mouseX = e.clientX;																			
							const mouseY = e.clientY;																			
							tooltip2.style.left = mouseX + 'px';																			
							tooltip2.style.top = mouseY + 'px';																			
							tooltip2.textContent = "동측G " + i + "번 좌석";
